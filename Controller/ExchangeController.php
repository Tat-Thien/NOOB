 ...
 */
class ExchangeController extends NoobBundleController
{
	 ...
	 * @REST\QueryParam(map=true, name="updated")
	 * @REST\QueryParam(name="updated[from]", description="Filter updated from")
	 * @REST\QueryParam(map=true, name="created")
	...
	public function cgetAction(ParamFetcherInterface $paramFetcher)
	{
		...
		$qb->select('e')
			->from('EntityBundle\Entity\Exchange', 'e');
	...
	public function getAction(ParamFetcherInterface $paramFetcher, $exchangeId)
	{
		$this->checkAuthentication($paramFetcher);
		$em = $this->getDoctrine()->getManager();
		$entity = $em->getRepository('EntityBundle\Entity\Exchange')->findOneById($exchangeId);
		if ($entity) {
			return $entity;
		} else {
